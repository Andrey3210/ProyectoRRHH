...
// Memoriza la URL base normalizada para evitar re-evaluaciones y choques de nombres
const API_CLIENT_BASE_URL = (() => {
  if (typeof window !== 'undefined' && window.__API_CLIENT_BASE_URL__) {
    return window.__API_CLIENT_BASE_URL__
  }
  ...
  const computed = url.toString().replace(/\/+$/, '')

  if (typeof window !== 'undefined') {
    window.__API_CLIENT_BASE_URL__ = computed
  }
